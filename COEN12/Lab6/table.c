...
//static recursive function to sort an array O(n log n)
static void quickSort(SET *sp, void **ret, int low, int high){


	//sets intital varaibles
	int pivot, i, j;
	void *temp;

	//checks if the array is already sorted(base case)
	if(low < high){

		//sets ints to passes values
		pivot =  low;
		i = low;
		j = high;

		//finds median by looping through
		while(i < j){

			//counts elements lower than pivot
			while((*sp->compare)(ret[i], ret[pivot]) <= 0 && i < high){
				i++;
			}	
			//counts elements higher than pivot
			while((*sp->compare)(ret[j], ret[pivot]) > 0 && j >= low){
				j--;
			}
	...
	//recursively does the same with left and right sides of array
	quickSort(sp, ret, low, j - 1);
	quickSort(sp, ret, j + 1, high);
	
	}
}

//returns an array of sorted elements O(n)
void *getElements( SET *sp){

	//allocate the returning array
	void **ret = malloc(sizeof(void*) * sp->count);

	//loops through table to find all filled elements and adds them to array
	int i, j;
	...
	//sorts the array then returns it
	quickSort(sp, ret, 0, sp->count - 1);
	return ret;
}
